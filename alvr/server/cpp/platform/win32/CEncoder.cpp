...
#include "../../analyze_used/helper_f.h"
		...
		void CEncoder::Run()
		{
			...
			while (!m_bExiting)
			{
				...
				{
					bool test = m_videoEncoder->Transmit(m_FrameRender->GetTexture(false).Get(), m_presentationTime, m_targetTimestampNs, m_scheduler.CheckIDRInsertion(), false);
					if(test){
						m_videoEncoder->Transmit(m_FrameRender->GetTexture(false).Get(), m_presentationTime, m_targetTimestampNs, true, true);
					}
				}
				m_encodeFinished.Set();
			}
