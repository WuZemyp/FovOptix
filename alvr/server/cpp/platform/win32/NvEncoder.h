...
public:
    ...
    */
    virtual bool EncodeFrame(std::vector<std::vector<uint8_t>> &vPacket, NV_ENC_PIC_PARAMS *pPicParams = nullptr, int states = 1, uint64_t timestamp = 0, float normalized_value = 0.0, float TPS=0.0, bool ReEncoding = false);
...
protected:
    ...
    *         NVENC hardware.
    */
    NVENCSTATUS DoEncode(NV_ENC_INPUT_PTR inputBuffer, NV_ENC_OUTPUT_PTR outputBuffer, NV_ENC_PIC_PARAMS *pPicParams, int states, uint64_t timestamp, float normalized_value);

    int8_t* GetQpMapLevel(int QpMapSize, int states, float normalized_value);
...
private:
    ...
    */
    bool GetEncodedPacket(std::vector<NV_ENC_OUTPUT_PTR> &vOutputBuffer, std::vector<std::vector<uint8_t>> &vPacket, bool bOutputDelay, bool saveFrame, int count, bool open, float TPS, bool ReEncoding);
